...
export function createCapsule(taskName) {
    initCapsules();
...
export function findCapsuleFile(nameOrSlug) {
    ...
    if (found)
        return path.join(CAPSULE_DIR, found);
    throw new Error(`Capsule not found: ${nameOrSlug}`);
}
export function loadCapsule(nameOrSlug) {
...
export function buildAgentPrompt(capsule) {
...
function sectionList(sections, key) {
    return (sections.get(normalizeHeading(key)) || [])
        .map((line) => line.trim())
...
function sectionNumber(sections, key) {
    const raw = sectionText(sections, key);
    const match = raw.match(/\d+/);
    return match ? Number(match[0]) : null;
}
function sectionBoolean(sections, key) {
...
function defaultCapsule(title, slug) {
