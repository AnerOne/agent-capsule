...
function commandNew(ctx: CommandContext): void {
  const taskName = ctx.args.join(' ').trim();
  if (!taskName) throw new Error('Usage: agent-capsule new "task name"');
...
function commandList(): void {
  ...
  for (const capsule of capsules) console.log(capsule);
}

function commandPrompt(ctx: CommandContext): void {
  const name = requireCapsuleArg(ctx);
  const capsule = loadCapsule(name);
  console.log(buildAgentPrompt(capsule));
}

function commandCheck(ctx: CommandContext): void {
  const name = requireCapsuleArg(ctx);
...
function commandRetry(ctx: CommandContext): void {
  const name = requireCapsuleArg(ctx);
  const capsule = loadCapsule(name);
...
function requireCapsuleArg(ctx: CommandContext): string {
